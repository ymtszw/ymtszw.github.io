...
- **ガイドラインへの追記を提案する**: 人間の開発者から追加の指示や新しい知見が共有された際、それが既存のガイドライン（`docs/GUIDELINES.md`）や実装計画（`docs/implementation-plans/`配下）に追加すべき内容であれば、積極的に追記を提案すること。これにより、プロジェクトのノウハウが蓄積され、将来のセッションでも活用できる

### コード品質管理

- **elm-formatの適用**: Elmコードを編集した後は、必ず`npx elm-format --yes <ファイル名>`でフォーマットを適用すること
  - **重要な注意点**: elm-formatは行末コメント（`-- コメント`）を次の行の先頭コメントに移動させることがある
  - 例: `|> List.take 10 -- コメント` → `-- コメント` が次の行に移動
  - コメントの意図を保つため、フォーマット後は必ずコメント位置を確認し、必要に応じて適切な位置（パイプライン処理の前など）に移動すること
- **ビルド検証**: コード変更後は`npm run build`でビルドが成功することを確認する
- **テスト実行**: 既存機能への影響を確認するため、適宜`npm run test`を実行する
...
- 2025-12-11: ガイドラインへの追記提案を実装アプローチに追加
